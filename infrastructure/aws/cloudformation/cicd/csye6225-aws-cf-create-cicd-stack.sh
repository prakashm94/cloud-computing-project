...
stackId=$(aws cloudformation create-stack --stack-name $stack_name --template-body file://csye6225-cf-cicd.json --capabilities CAPABILITY_NAMED_IAM --parameters  ParameterKey=resource2,ParameterValue=$resource2 ParameterKey=resource3,ParameterValue=$resource3 ParameterKey=resource4,ParameterValue=$resource4 ParameterKey=resource5,ParameterValue=$resource5 ParameterKey=domainName,ParameterValue=$domain_name --query [StackId] --output text)
